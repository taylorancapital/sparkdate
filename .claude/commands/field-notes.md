...
Six sources. Sweep all six; none alone is sufficient.
...
1. **The memory files** —
   ...
   section that is usually a failure written up at the time. Read
   `MEMORY.md` for the index, then every file whose `modified:` stamp is after
   the watermark.
