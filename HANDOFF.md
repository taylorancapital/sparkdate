...
- **Seating women who arrived together is built (#488), and the admin still
  needs one line to read the Eventbrite half of the input (#491).** (09-10)
  Taylor asked for both. The seating is done and tested — quotas and the men's
  partition are untouched, and with no recorded parties the seating is
  byte-identical to before. #491 starts storing Eventbrite's `order_id`, which
  is how ~37% of women could be grouped automatically; before it, only 18%
  carried any party key at all and 47% walk up to the door
  (see [[who-arrived-together-is-not-in-the-data]]). **Next step: whichever of
  #488 / #491 merges SECOND needs `public/admin.html`'s party seeding changed
  from `r.paymentIntentId` to `r.paymentIntentId || r.orderId`** — one line, in
  the `parties` loop where the roster is assembled. Until that lands the two
  PRs are each half of a feature. Deliberately not stacked: they touch
  different files and #488's region is the one that would have conflicted.

- **A standing 1-on-1 segment leaves 38% of a skewed room with nobody to talk
  to, and the screen does not name them.** (09-10) Taylor's call is that the
  tables stay in the room but standing is likely better for the 7-minute
  1-on-1s. Measured against real rosters: Round 2 (9W/20M) and Good Good
  (4W/9M) both leave 38% of the room unpaired in EVERY 1-on-1 round, all men;
  a balanced 10/10 leaves nobody. Seated that reads as normal, standing it does
  not. This also retires fix 4 from `reports/CHEMISTRY_RUN_OF_SHOW_AUDIT_2026-09-09.md`
  — giving a 1-on-1 pair a table number is wrong for a standing format, and
  what is actually needed is fix 1 (per-person "who is next" through all 9 plan
  steps) plus surfacing the idle list so the host can park them at a table.
  **Next step: Taylor has been asked whether to build fix 1 + the idle list;
  nothing is started.**
