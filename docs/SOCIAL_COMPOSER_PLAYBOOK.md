# Meta Business Suite / TikTok Studio composer playbook (historical, Jun–Aug 2026)

Moved out of `CLAUDE.md` on 2026-09-11. It was 66% of that file and loaded
into every session in every domain; nothing below applies to queued posts.

**Status.** Since 2026-08-21 the social calendar is `content/queue.csv` +
`content/brand.json` and posting is `scripts/social.js` — see
`docs/SOCIAL_RUNBOOK.md` and the "SOCIAL POSTING" section of `CLAUDE.md`.
This document is the procedure for the one thing that pipeline does not
cover: a post made by hand in the platform's own composer — an unplanned
post, a live event story, an IG Story with a link sticker. The standing rules
it established (Claude never clicks Publish / Schedule / Finish later on
Meta; TikTok direct-Schedule is accepted; never invent a number) live in
`CLAUDE.md`, not here.

What is history and should not be acted on: the 30-Day Content Plan
docx/pdf as a source of truth, the `SparkDate_Week[N]_Content_Schedule.docx`
deliverable, the C1–C6 asset code map (Event 2, July 2026), the
`week<N>_<Theme>` UTM convention (now `<EVENT>_<YYYYMM>`, built by
`scripts/ad-utm.js`), and the Week 3 / Jul 8–14 build logs. They are kept
verbatim because the composer quirks they record — video-attach splitting a
combined FB+IG draft, the Create Reel wizard, the TikTok scroll-wheel time
picker, hashtag-autocomplete corruption — were learned the hard way and are
still true of those UIs.
