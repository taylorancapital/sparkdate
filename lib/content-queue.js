...
// The unfilled placeholder in a row's PUBLISHING text, or null.
//
// Recap rows are written with the attendance figure left as `[REAL NUMBER]`,
// because the rule is never to estimate it -- so the copy is finished days
// before the number exists. `notes` is where the instruction to fill it lives
// and never publishes, so it is deliberately not examined; caption, caption_x
// and hashtags all do.
function unfilledPlaceholder(row) {
  for (const field of ['caption', 'caption_x', 'hashtags']) {
    const m = /\[[A-Z][A-Z0-9 _#-]*\]/.exec(String(row[field] || ''));
    if (m) return m[0];
  }
  return null;
}
...
module.exports = {
  ...
  isSchedulable, isFeedPost, unfilledPlaceholder,
};
