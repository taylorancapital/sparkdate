...
function isAllowed(origin) {
  ...
  //
  // The label pattern matches Vercel's: alphanumeric segments separated by
  // single dashes — never consecutive (audit L1). The old [a-z0-9-]+
  // permitted "sparkdate--evil.vercel.app" which Vercel itself never
  // generates but third parties could.
  if (/^https:\/\/sparkdate(-[a-z0-9]+(?:-[a-z0-9]+)*)?\.vercel\.app$/i.test(origin)) return true;
  return false;
}
