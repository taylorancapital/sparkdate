...
async function enrollEventbriteOne({ email, name, gender, eventId, eventName, priceCents, channel, ebFeeCents, orderId }) {
  ...
  // The marketplace order this ticket was bought on. Two attendees sharing it
  // came in one checkout, which is how the seating knows to sit friends
  // together (public/admin.html, _chemGroups). Same "absent means not known"
  // discipline as ebFeeCents: only written when the caller actually has it, so
  // a re-import from a source that does not supply it cannot blank it out.
  ...
  try {
    ...
    if (!leadDupe.empty) {
      ...
      // Fill the gender ONLY when blank — the same rule the user doc follows
      // above, for the same reason: records were hand-corrected on 2026-08-30
      // and overwriting a correction with a ticket-class inference is worse
      // than the blank it replaces. hasGender() is used rather than a truthy
      // test because a value round-tripped through a sheet can arrive as the
      // literal string "null".
      if (gender && !hasGender(leadDupe.docs[0].data().gender)) {
        await leadRef.update({ gender });
      }
    }
    if (leadDupe.empty) {
      ...
      await db.collection('leads').add({
        ...
        email: norm,
        phone: '',
        // Already normalised at the top of this function. `leads` is what the
        // nurture/newsletter passes read, and it carried no gender from any
        // write path — so an Eventbrite buyer whose gendered ticket class we
        // DID resolve still arrived there indistinguishable from an anonymous
        // exit-intent capture. Written even when null so the field is present.
        gender: gender || null,
        source: chan.source,
        referredBy: null,
        createdAt: FieldValue.serverTimestamp(),
