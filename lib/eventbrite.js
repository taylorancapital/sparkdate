...
// Python's repr() of a bytes object, e.g. b'Jordan'. On 2026-08-30 two live
// Eventbrite signups landed as `b'Jordan' b'Reyes'` and `b'Alexis'
// b'Moore'`. JavaScript has no such spelling, so nothing in this codebase
...
// people were greeted as "b'Jordan'" and "b'Alexis'".
//
...
// The order this attendee was bought on. Two attendees sharing it were bought
// in ONE checkout, which is the only way imported data can know they arrived
// together — and knowing that is what lets the seating sit friends at the same
// table (public/admin.html, `_chemGroups`).
//
// Worth capturing because the alternative covers almost nobody: before this,
// the only party key in event_registrations was a Stripe paymentIntentId,
// carried by 18% of women ever registered and identifying exactly one pair in
// the whole history. Eventbrite is 37% of women, and this id was already on
// every attendee record the sync fetched — read off the wire and dropped.
//
// Coerced to a string. Eventbrite has returned order ids as both numbers and
// strings across API versions, and a party key where 12345 !== "12345" does
// not error — it silently stops grouping anyone, which is the failure this
// whole area keeps producing.
function attendeeOrderId(a) {
  a = a || {};
  const v = a.order_id != null ? a.order_id : (a.order && a.order.id);
...
// Does a stored record already carry a gender?
//
// The backfill's single most important rule is that it only ever fills BLANKS.
// Records were hand-corrected on 2026-08-30, and overwriting somebody's
// correction with a value inferred from a ticket class would be a worse bug
// than the blank it replaces. The string forms matter: a field that went
// through JSON or a spreadsheet can arrive as the literal "null"/"undefined",
// which is blank in every sense except the one `if (d.gender)` tests.
function hasGender(v) {
  const s = String(v == null ? '' : v).trim().toLowerCase();
  return s !== '' && s !== 'null' && s !== 'undefined';
}

/**
 * email -> 'woman' | 'man' for a list of Eventbrite attendees.
 *
 * An email that appears under two different gendered ticket classes is a
 * contradiction, and this REFUSES it rather than letting arrival order decide:
 * a repeat buyer resolved by whichever record Eventbrite happened to return
 * last is exactly the kind of silent wrong answer that is worse than a blank.
 * Conflicts come back separately so the caller can report them.
 */
function genderByEmail(attendees) {
  const byEmail = new Map();
  const conflicts = new Set();
  for (const a of attendees || []) {
    const email = normalizeEmail(a && a.profile && a.profile.email);
    const g = attendeeGender(a);
    if (!email || !g) continue;
    if (byEmail.has(email) && byEmail.get(email) !== g) conflicts.add(email);
    byEmail.set(email, g);
  }
...
module.exports = { EB, normalizeEmail, cleanName, normalizeGender, attendeeGender, attendeeOrderId, hasGender, genderByEmail, ebFetch, ebGetAll, RETRYABLE };
