...
// How long a past event keeps its /l/ routes. See recentPastEvents().
const PAST_EVENT_GRACE_DAYS = 365;

/**
 * Past events that still need their short links to resolve.
 *
 * WHY THIS EXISTS. fetchUpcomingEvents() drops an event the moment its start
 * time passes, so on 2026-09-09 a --write deleted all 19 /l/mc-* routes the
 * morning after Marion Court ran. Those links were not dead weight: they were
 * live inside listings on Patch, AllEvents and Nextdoor, which stay up until
 * somebody takes them down by hand. Deleting the redirect turned every one of
 * them into a 404.
 *
 * A 404 is the worst of the available options. The event page itself still
 * serves 200 for a past event and says so ("no longer", "ended") with the
 * upcoming events one click away, so pointing a stale listing there recovers
 * the visitor. Pointing them at nothing does not.
 *
 * These come from content/brand.json, NOT the sitemap: /sitemap.xml lists
 * upcoming events only, so by the time this matters the event is already
 * unfetchable. brand.json is the durable record and has the event_id and date
 * this needs.
 *
 * The window is bounded because Vercel caps a project at 1,024 redirects and
 * this generates ~18 per event. A year is ~450 rows at this event cadence,
 * comfortably inside the cap and longer than any listing has stayed live.
 */
function recentPastEvents(b, now = new Date(), graceDays = PAST_EVENT_GRACE_DAYS) {
  const cutoff = new Date(now.getTime() - graceDays * 86400000);
  const out = [];
  for (const [, ev] of Object.entries((b || brand()).events || {})) {
    if (!ev.event_id || !ev.date) continue;
    // Noon UTC: far enough from either month boundary that the America/New_York
    // offset campaignFor() applies can never roll the month over.
    const start = new Date(`${ev.date}T12:00:00Z`);
    if (start >= now || start < cutoff) continue;
    out.push({ id: ev.event_id, url: `${SITE_ORIGIN}/event?id=${ev.event_id}`, start });
  }
  return out;
}
...
module.exports = {
  ...
  recentPastEvents, PAST_EVENT_GRACE_DAYS,
};
