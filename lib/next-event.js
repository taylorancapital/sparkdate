...
//
// TWO, not three. Corrected 2026-09-01: events run 6:30 to 8:30 PM. The old
// 3 put an hour that does not exist into the schema.org `endDate` of every
// event page -- which is the block Google reads to print event times in
// search results, and what any calendar scraping the site ingests. No event
// doc sets durationHours, so this constant WAS the published end time.
//
// It also decides when an event stops being "current". At 3 the site kept
// selling and check-in kept pointing at a room that had emptied an hour
// earlier. See reports/FACT_AUDIT_2026-09-01.md section 1.
//
// scripts/audit-checkin-event-drift.js widens this with its own grace on
// both ends, so narrowing it here does not lose late check-ins.
