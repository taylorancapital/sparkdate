...
function seatFields(event, gender) {
  ...
  // Case-insensitive on purpose. The Eventbrite import stored gender
  // capitalized ("Woman"/"Man") for months — 63 of 104 tickets — while the
  // site's own checkout stores lowercase. An exact match here sent every
  // imported woman to the MEN'S counter on a legacy split event, which is
  // the seat-inventory equivalent of the GA4 "Facebook / facebook" bug.
  // The import now normalizes at write and the backfill lowercased the
  // stored docs, but this is the money path, so it defends itself too.
  return String(gender || '').toLowerCase() === 'woman'
...
function ticketPriceDollars(event, gender) {
  ...
  const legacy = String(gender || '').toLowerCase() === 'woman'
