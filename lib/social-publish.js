...
function planRow(row, surface, nowMs, opts = {}) {
  ...
  // No artwork, no post. A row can be approved weeks before its art exists --
  // live-coverage rows are shot on the night and never have it in advance --
  // and handing Facebook a scheduled post with no images would either fail at
  // the API or, worse, publish an empty one. Approval says "this content is
  // authorised"; it does not say "this content is ready".
  if (!String(row.asset_files || '').trim()) {
    return { action: 'skip', reason: 'no artwork yet' };
  }

  // A feed post needs a feed-shaped image. When every asset is 1080x1920 the
  // request builder falls back to using them anyway -- right in general, since
  // a cropped image beats no post -- so a "Single image + Story" row delivered
  // as two story frames would schedule a Facebook post carrying two vertical
  // images. Refuse it here instead. The linter only warns about this; the
  // block belongs at the point of publishing.
  if (surface === 'fb' || surface === 'ig') {
    const files = String(row.asset_files || '').split(',').map((f) => f.trim()).filter(Boolean);
    const isReel = /reel/i.test(row.format || '');
    // _tt art is TikTok's alone (see assetUrls' notTikTok) and must not count
    // as feed-shaped art just because it fails the _story test. A row
    // carrying only _tt and _story files -- MC-12 did, from orphaned TikTok
    // exports left behind after its source art changed -- has NO feed asset
    // at all, and slipped past this guard because not every RAW file matched
    // _story. Judge the guard on what a feed post could actually use.
    const feedCandidates = files.filter((f) => !/_tt\./i.test(f));
    if (!isReel && feedCandidates.length && feedCandidates.every((f) => /_story\./i.test(f))) {
      return { action: 'skip', reason: 'every asset is story-shaped -- needs a 1080x1080 for the feed' };
    }
  }
