...
function assetUrls(row, baseUrl, surface) {
  ...
  // `_tt` art is TikTok's alone. Every other surface's "use everything when
  // nothing matches" fallback predates it and would happily serve it:
  // LX-24 carries only _story and _tt files, so the feed filter emptied and
  // the fallback put VERTICAL art in a Facebook post. Excluding it from the
  // fallbacks is the fix; excluding it from the primary filters is not
  // enough, because the fallbacks are what actually fire on those rows.
  const notTikTok = (f) => !/_tt\./i.test(f);

  let picked = all;
  if (surface === 'ig_story') {
    const story = all.filter((f) => /_story\./i.test(f));
    picked = story.length ? story : all.filter(notTikTok);
    ...
    const feed = all.filter((f) => !/_story\./i.test(f) && notTikTok(f));
    picked = feed.length ? feed : all.filter(notTikTok);
  }
