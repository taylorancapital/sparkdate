...
async function uploadBytes(uploadUrl, buf, mime, { fetchImpl = fetch } = {}) {
  ...
  const res = await fetchImpl(uploadUrl, {
    ...
    headers: {
      'Content-Type': mime,
      // Content-Length is deliberately NOT set. undici derives it from the
      // Buffer, and setting it by hand on a fetch() is at best redundant and
      // at worst a TypeError on a forbidden header -- which would kill the
      // function before any of our error handling ran.
      //
