...
| Phase | Window | Days | Daily budget | Cold share | Cold, broad $ | 2-for-1 $ | Retarget $ |
|---|---|--:|--:|--:|--:|--:|--:|
| **Seed** | T-21 to T-15 | 7 | $10.00 | $8.00 (80%) | $6.00 | $2.00 (25% of cold, exact Meta floor) | $2.00 (20%, exact Meta floor) |
| **Build** | T-14 to T-8 | 7 | $14.00 | $8.40 (60%) | $6.30 | $2.10 (25% of cold) | $5.60 (40%) |
| **Close** | T-7 to T-0 | 8 (incl. event day) | $16.00 | $5.60 (35%) | $3.60 | $2.00 (25% is $1.40; floored) | $10.40 (65%) |

Three campaigns, one ad set each, budget on the campaign: `<Event> | Cold` (broad), `<Event> | 2-for-1` (women-locked, the one exception in §8.3), `<Event> | Retargeting` (broad). The 2-for-1 column is carved out of the cold share only once the event's `two_for_one` entry is registered; Loxleys' pre-carve-out pair keeps the full cold share. *(Columns rewritten 2026-09-10.)*
...
**Campaign structure — build all three campaigns once, on day one of Seed, never recreate:** objective, `optimization_goal`, `attribution_spec`, and creative `url_tags` are frozen at birth and cannot be edited on a live ad set. `[platform]` Build `<Event> | Cold`, `<Event> | 2-for-1` and `<Event> | Retargeting` on day one — all `OFFSITE_CONVERSIONS` / `PURCHASE` pixel / 7-day click / Advantage+ audience **off**, budget on the campaign with one ad set each (the only shape the ladder can move), matching the account's already-converged-on standard — and move between phases by editing daily budget only. This is exactly what PR #454's ladder registry already does with existing campaigns (a budget-JSON edit, not a rebuild).
...
### 8.3 Gender targeting — one gender-restricted ad set, the 2-for-1 cell, and nothing else

> **Rewritten 2026-09-10.** Taylor's decision, option A of `reports/TWO_FOR_ONE_TO_EVERYONE_2026-09-10.md`, after two of the four points the 09-06 rule rested on were found not to hold. What was retired, and why, is recorded at the end of this section rather than deleted.

**Run every ad set broad — cold and retargeting, every phase — with one exception: a women-locked cold ad set, gender expansion off, that is the only place the 2-for-1 creative runs.** `[judgment]` on the rule; `[measured]` on the three facts it rests on, all from 44 delivered ads, lifetime, grouped by the gender Meta actually delivered to rather than the targeting label:

1. **A broad ad set buys men.** Every broad cold sales cell on this account spent 60–68% on men (women's share of spend 31.7 / 34.6 / 37.5 / 40.4%). A woman's impression costs ~46% more than a man's ($32.42 vs $22.26 CPM), and women's CPM was above men's in 21 of 22 ads where both genders were served (sign test p ≈ 10⁻⁵), so an outcome-optimised auction drifts male. Cost per woman landing on the site is the same locked or broad ($1.64 vs $1.61), so a women-locked cell lands **2.7× the women per dollar** (6.1 vs 2.3 per $10). Per $100: 61 women locked; 23 women and 36 men broad. Direction solid; the magnitude rests on 4 cells from 3 campaigns.
2. **The 2-for-1 creative does not keep men out.** The two times it reached men ($45.61), they clicked it at 83% and 96% of women's link-click rate — no detectable difference on 2,317 male impressions, enough to rule out repulsion, not a 20–30% gap — and produced the account's only Meta-attributed 2-for-1 purchase.
3. **The +1 mirrors the buyer.** 7 of 7 plus-ones matched the buyer's gender (Firestore, 09-08; exact 95% lower bound 59%). A man shown the offer brings a man.

The 2-for-1 is therefore the one creative that cannot be shown to men without buying men, and the women-locked cell exists for it alone.

**The rule, cell by cell:**
- **`<Event> | Cold`, broad ad set** — the social-proof/testimonial creative (`playbook_v2.creative`), for volume, knowing it spends ~63% on men. No 2-for-1 line in its copy, ever (`caption_rules.banned_outside_female_ad_set`).
- **`<Event> | 2-for-1`, the cell** — its own campaign, one women-locked ad set: `genders: [2]`, `targeting_automation.individual_setting.gender = 0` (the expansion flag; `advantage_audience: 0` alone does not cover it, and this flag is how Good Good's "women" set delivered 64% to men), the 2-for-1 creative only. Budget on the campaign like the other two, so the ladder can move it: **25% of each phase's cold amount** (`playbook_v2.two_for_one_of_cold`), never below the $2.00/day floor `[platform]` — $2.00 / $2.10 / $2.00 (floored) across Seed / Build / Close at the reference total, with broad cold keeping $6.00 / $6.30 / $3.60. The 25% is `[judgment]`: it lands exactly on the floor at Seed, the same way 20% retarget does at $10; the split inside cold is untested.
- **`<Event> | Retargeting`** — broad, always, and on a better reason than the 09-06 text gave: the worst retargeting result on record was a women-only pool, `MC Retargeting`, which reached 407 people at frequency 11.67 for $103.14 and zero sales. A warm pool is small by construction; halving it by gender is how you reach the same people eleven times.

**Built, not by hand.** `node scripts/build-paid-campaign.js --event=<KEY> --execute` builds all three campaigns PAUSED, budget on each campaign, one ad set each, and reads every one back: the cell must come back with `genders: [2]` and no expansion flag. Register the three ids as roles `cold`, `two_for_one` and `retargeting` in `content/paid-campaigns.json`; the ladder carves the cell's share out of cold only once that entry exists, so Loxleys' pre-carve-out pair keeps its full cold rate. `--no-two-for-one` builds the old pair for an event whose 2-for-1 already runs elsewhere. What is still by hand is the 2-for-1 **ad** itself: `playbook_v2.creative` has no 2-for-1 entry, and no v2 attach script exists for any role. The check already exists: `npm run ads:review` lists every 2-for-1 ad that has ever reached men and exits 3 when one of them is still delivering, or when any ACTIVE ad set carries gender expansion — because a written rule did not catch the $63.73 Good Good leak and a check would have. The two archived 2026-08 leaks stay on the record without failing every run.

**Live campaigns on 09-10:** Loxleys' `female | Sales` ad set carries the 2-for-1 and is, in effect, the cell. Its `male | Sales` set is the one that no longer matches; it should be the broad set with the social-proof creative. Marion Court has ended.

**Retired from the 09-06 text, and why:**
- ~~"A `PURCHASE`-optimized ad set restricted to one gender frequently can't find enough qualifying impressions to spend its budget" (old point 4, called load-bearing).~~ Refuted twice: a women-only set spent 101% of its budget on 09-08, and `Marion Court | female | close 2for1` spent $41.74 in its last four days against $14/day. The two cells that did starve shared a $5.11/day CBO on the floor; that is budget, not the gender checkbox.
- ~~"Male LPV→purchase conversion matches female's, so there is no evidence targeting women outperforms reaching everyone" (old point 2).~~ True, and beside the point. Conversion *rate* per visit is similar; the *number* of women who visit per dollar is 2.7× higher locked. The rate is not the lever when the share is.
- The n = 6 correlation (old point 1, ρ = −0.657, p = 0.088) and the organic channel's 35.7% women (old point 3) stay as caveats, not bases. The first is confounded: Good Good is the only Philadelphia event and the one whose "women" cells delivered 64% men through gender expansion. The second is about a channel, not an ad set; a broad Meta ad set delivered 32–40% women on every sales cell.
- "The lever is what the ad shows, not who it's aimed at" is half right and stays as half. What the ad shows moves women's checkouts (Tellus, same dates: the broad social-proof cell got 5 women's checkouts to the locked 2-for-1 cell's 1, from fewer visits). Who it is aimed at decides whether women see it at all. Both levers exist; the creative does not replace the ad set.

**Not verified, and where to read it:** creative and targeting are entangled on this account — every 2-for-1 ad ran women-locked and every broad ad ran social-proof copy — so the reach finding is measured and the conversion half is not. The Firestore figures were measured 09-08 and not re-read on 09-10. Option B of the 09-10 report, a same-event A/B read in Firestore, is the only way to separate creative from targeting; the account will not produce that comparison by accident. `reports/TWO_FOR_ONE_TO_EVERYONE_2026-09-10.md` §7–§8.
...
- **The gender rule was rewritten 09-10 (§8.3) and now rests on delivery data; the under-spending claim it used to rest on was refuted.** What is still unmeasured: the 25% split inside cold, and whether the 2-for-1 creative converts women better locked than a social-proof creative does broad — creative and targeting are entangled on this account, and only a same-event A/B (option B of `reports/TWO_FOR_ONE_TO_EVERYONE_2026-09-10.md`) separates them.
...
2. Run `node scripts/build-paid-campaign.js --event=<KEY> --execute`. It builds three sales-objective campaigns (`<Event> | Cold`, `<Event> | 2-for-1`, `<Event> | Retargeting`) on day one, all `OFFSITE_CONVERSIONS`/`PURCHASE`/7-day-click/Advantage+ off, budget on each campaign per the Quick-reference table with the $2.00 floor-priority rule (§8.2) applied. Register all three ids in `content/paid-campaigns.json`, one entry per role.
3. Cold and Retargeting are broad. The 2-for-1 campaign is women-only with gender expansion off, and it is the only place the 2-for-1 creative goes (§8.3). No other gender-restricted ad set.
