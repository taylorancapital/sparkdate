...
|---|---|--:|--:|--:|--:|---|
| **Seed** | T-21 to T-15 | 7 | $10.00 | $8.00 (80%) | $2.00 (20%, exact Meta floor) | broad, plus the women-locked 2-for-1 cell (§8.3) |
| **Build** | T-14 to T-8 | 7 | $14.00 | $8.40 (60%) | $5.60 (40%) | broad, plus the women-locked 2-for-1 cell (§8.3) |
| **Close** | T-7 to T-0 | 8 (incl. event day) | $16.00 | $5.60 (35%) | $10.40 (65%) | broad, plus the women-locked 2-for-1 cell (§8.3) |
...
- **`<Event> | Cold`, the 2-for-1 cell** — `genders: [2]`, `targeting_automation.individual_setting.gender = 0` (the expansion flag; `advantage_audience: 0` alone does not cover it, and this flag is how Good Good's "women" set delivered 64% to men), the 2-for-1 creative only, its own ABO budget funded out of the phase's cold share and never below the $2.00/day floor `[platform]`. At Seed's $8/day cold share that is $2–4/day; the split inside cold is `[judgment]`, untested.
...
**What still has no builder.** `scripts/build-paid-campaign.js` builds one broad ad set per role and asserts `genders` is undefined on read-back; `playbook_v2.creative` has no 2-for-1 creative. Until both exist the cell is built by hand on the `scripts/meta-create-lx-sales-campaign.js` pattern (female ad set, expansion off, targeting read back after creation). The check already exists: `npm run ads:review` lists every 2-for-1 ad that has ever reached men and exits 3 when one of them is still delivering, or when any ACTIVE ad set carries gender expansion — because a written rule did not catch the $63.73 Good Good leak and a check would have. The two archived 2026-08 leaks stay on the record without failing every run.
