...
**Mechanism 1 — a brand-new, zero-converting campaign is diluting paid social, and it has already been rebuilt.** `ga4-api-daily-by-source` shows `Facebook / paid_social` falling from 432 to 260 sessions week-over-week — read alone, that looks like a real paid-social decline. Most of it is not. Day-by-day, the drop is a cliff exactly at 2026-09-01 (86→107→57→**3→1→4→2**, 08/29–09/04). Two lowercase-tagged rows account for a large piece of the gap, but they are not the same kind of row: `ga4-api-utm-ad-detail` shows `ig / paid_social` (127 recent-week sessions) has **only ever been used by `LX_202609`** — 126 of its 129 window-wide sessions — **Loxleys' prime-rate Traffic ads, live since 2026-08-30 per the commit history.** `fb / paid_social` (102 recent-week sessions) is messier: it is a pre-existing fragmented tag several older campaigns already used sporadically (Marion Court Retargeting, Good Good, Tellus), to which Loxleys added roughly 90 of its 209 window-wide sessions from 08-30 onward — it also carries an unrelated 88-session spike on 2026-08-22 that predates Loxleys by over a week and cannot be attributed to it. Excluding that one unrelated spike, combined Facebook+Instagram traffic (both casings) held roughly flat week-over-week (642 → 689, +7%); including it, the honest comparison reads as a small decline (730 → 689, −6%) with its own separate, unexplained 08-22 cause. Either way, the 432→260 `Facebook / paid_social` figure alone overstates the real change — a meaningful share of it is relabeling, not lost reach. And every `ig`/`fb` lowercase-tagged session this week carries **zero key events** — this new traffic converts at 0%, which is exactly why it drags the aggregate rate down even though total volume didn't meaningfully fall. This is not a live open question: commit `600a0646` (2026-09-05 14:02 ET, PR #450) independently found the same thing — "$18.04, 183 link clicks, zero tickets in seven days" — and paused Loxleys' Traffic campaign in favor of a rebuilt Sales-objective one the same afternoon (see ALSO #1). I did not re-verify the Meta/Firestore side of that commit; I cite it only to explain why tonight's GA4 numbers look the way they do.

**Mechanism 2 — Eventbrite listing traffic fell off a cliff on 09-01, and nothing in this pull explains why.** `eventbrite / listing` is one of the property's best-converting channels (window-wide: 13.2% conversion, $290.39 revenue, 11 of 39 transactions — see TRAFFIC below). Day-by-day it goes 4→4→3 (08/29–08/31) then **0→2→1→1** (09/01–09/04), a 77% week-over-week session drop (66→15) with the same 09-01 cliff date as Mechanism 1, but no tagging-fragmentation explanation fits — there is no new Eventbrite-adjacent row absorbing the missing sessions (`facebook_event / listing` and `nextdoor_event / listing` are new but tiny, +9 and +3 sessions). Total site sessions did not fall on 09-01 (109 that day per `daily-trend`, in line with neighboring days), so this looks like a real, specific drop in whichever Eventbrite listing(s) are currently live, not a measurement artifact. I have no Eventbrite listing URL on disk to check directly (none is hardcoded in `public/` or `content/`; ticket URLs live in Firestore, out of scope for this pull) — recommend checking the live listing(s) directly. See zero-risk fixes.
...
**The `fb / paid_social` row above is old, pre-existing fragmentation, not new** — it has appeared sporadically before (an unrelated 88-session spike on 2026-08-22, plus scattered use by Marion Court Retargeting, Good Good and Tellus). What's new is that Loxleys' Traffic ads (live since 08-30) started using it too, adding roughly 90 of its 209 window-wide sessions (HEADLINE, Mechanism 1) — Loxleys did not create this fragmentation, it added to it.
